// slightly edited version of https://github.com/tilkinsc/Lua.NET/blob/main/src/Lua54.cs
...
public static class Lua
{
	...
	private const string DllName = "lua5.4";
    ...
    public static byte[] lua_tobytebuffer(lua_State L, int idx)
    {
        size_t len = 0;
        nint ptr = _lua_tolstring(L, idx, ref len);
        byte[] rtv = new byte[len];
        Marshal.Copy(ptr, rtv, 0, (int)len);
        return rtv;
    }
	...
	private static extern nint _lua_pushlstring(lua_State L, byte[] s, size_t len);
	public static string? lua_pushlstring(lua_State L, string s, size_t len)
	{
		return Marshal.PtrToStringAnsi(_lua_pushlstring(L, Encoding.UTF8.GetBytes(s), len));
	}

    public static void lua_pushbytebuffer(lua_State L, byte[] s)
    {
        _lua_pushlstring(L, s, (ulong)s.Length);
    }
    ...
    public static byte[] luaL_checkbytebuffer(lua_State L, int arg)
    {
        size_t len = 0;
        nint ptr = _luaL_checklstring(L, arg, ref len);
        byte[] rtv = new byte[len];
        Marshal.Copy(ptr, rtv, 0, (int)len);
        return rtv;
    }
	...
	[DllImport(DllName, CallingConvention = Convention)]
	public static extern int luaL_loadbufferx(lua_State L, byte[] buff, size_t sz, string name, string? mode);
