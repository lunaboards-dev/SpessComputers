...
local function assert(ok, msg)
    if ok then
        ...
        test_fail(string.format("FAIL: %s", msg or "assertion failed!"))
    end
end

local function checkerror (msg, f, ...)
  local s, err = pcall(f, ...)
  assert(not s and string.find(err, msg), string.format("expected '%s', got '%s'", msg, err))
end
...
-- we don't pass this and currently this is a wontfix
...
-- reuse tests, can't pass, wontfix
